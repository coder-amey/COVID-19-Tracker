...
def plot_growth(df, figure, title):
	...
	lines = cases.plot(df['Date'], df['Confirmed'], 'yellow', df['Date'], df['Deaths'], 'red')
	cases.legend(lines, ('Confirmed',  'Dead'))
	
	lines = cases_log.plot(df['Date'], df['Confirmed'], 'yellow', df['Date'], df['Deaths'], 'red')
	cases_log.legend(lines, ('Confirmed',  'Dead'))

	lines = growth_rate.plot(df['Date'], get_rate(df['Confirmed']), 'yellow', df['Date'], get_rate(df['Deaths']), 'red')
	growth_rate.legend(lines, ('Confirmed',  'Dead'))

	lines = growth_rate_log.plot(df['Date'], get_rate(df['Confirmed']), 'yellow', df['Date'], get_rate(df['Deaths']), 'red')
	growth_rate_log.legend(lines, ('Confirmed',  'Dead'))
...
if __name__ == "__main__":
	...
	print("\nGenerating trends...")
	print("Current Tally:\t{}\nConfirmed: {}\tDead: {}".format(country.title(), tally['Confirmed'], tally['Deaths']))
